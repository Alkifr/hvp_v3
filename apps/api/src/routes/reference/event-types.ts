...
export const eventTypesRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/", async (req) => {
    ...
    return await app.prisma.eventType.create({ data: body });
  });
  ...
  app.patch("/:id", async (req) => {
    ...
    const body = z
      .object({
        ...
        color: z.string().trim().min(4).max(16).nullable().optional(),
        isActive: z.boolean().optional()
      })
      .parse(req.body);

    return await app.prisma.eventType.update({ where: { id }, data: body });
  });
