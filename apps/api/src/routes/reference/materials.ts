...
export const materialsRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", async (req) => {
    assertModelPermission(req as any, "Material", "view");
    return await app.prisma.material.findMany({ orderBy: [{ isActive: "desc" }, { code: "asc" }] });
  });

  app.post("/", async (req) => {
    assertModelPermission(req as any, "Material", "add");
    const body = z
      .object({
  ...
  app.patch("/:id", async (req) => {
    assertModelPermission(req as any, "Material", "change");
  ...
  app.delete("/:id", async (req) => {
    assertModelPermission(req as any, "Material", "delete");
    const id = zUuid.parse((req.params as any).id);
