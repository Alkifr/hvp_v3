...
export const hangarsRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", async (req) => {
    assertModelPermission(req as any, "Hangar", "view");
  ...
  app.post("/", async (req) => {
    assertModelPermission(req as any, "Hangar", "add");
    const body = z
      .object({
        code: z.string().trim().min(1).max(32),
        name: z.string().trim().min(1).max(200),
        station: z.string().trim().min(1).max(32).nullable().optional(),
  ...
  app.patch("/:id", async (req) => {
    assertModelPermission(req as any, "Hangar", "change");
  ...
  app.delete("/:id", async (req) => {
    assertModelPermission(req as any, "Hangar", "delete");
    const id = zUuid.parse((req.params as any).id);
