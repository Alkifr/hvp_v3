...
type WizardTab = "what" | "when" | "who";
type PageTab = "variants" | "history";
type SendStatus = "SENT" | "FAILED" | "EMPTY";
...
const LS_VARIANT = "mailDigest:selectedVariantId";
const LS_TAB = "mailDigest:pageTab";
...
function formatMskDay(iso: string): string {
...
function scheduleLabel(v: Pick<MailVariant, "scheduleMode" | "scheduleTime" | "scheduleWeekdays" | "scheduleMonthDay">): string {
  if (v.scheduleMode === "manual") return "Вручную";
  if (v.scheduleMode === "daily") return `Каждый день · ${v.scheduleTime}`;
  if (v.scheduleMode === "weekly") {
    const days = v.scheduleWeekdays.map((id) => WEEKDAYS.find((d) => d.id === id)?.label ?? id).join(", ");
    return `${days || "Дни не выбраны"} · ${v.scheduleTime}`;
  }
  return `${v.scheduleMonthDay} число · ${v.scheduleTime}`;
}
...
function copyName(name: string): string {
  const base = name.replace(/\s*\(копия(?: \d+)?\)\s*$/i, "").trim() || "Рассылка";
  return `${base} (копия)`;
}
...
export function MailDigestView() {
  ...
  const [pageTab, setPageTab] = useState<PageTab>(() => (readLs(LS_TAB) === "history" ? "history" : "variants"));
  const [wizardTab, setWizardTab] = useState<WizardTab>("what");
  const [selectedId, setSelectedId] = useState<string | null>(() => readLs(LS_VARIANT));
  const [hydratedId, setHydratedId] = useState<string | null>(null);
  const [name, setName] = useState("Изменения плана ТО");
  ...
  const [histStatus, setHistStatus] = useState<SendStatus | "all">("all");
  const [histChannel, setHistChannel] = useState<"all" | "manual" | "schedule">("all");
  const [histVariantId, setHistVariantId] = useState<string>("all");
  ...
  const listQ = useQuery({
    queryKey: ["mail-digest", "list"],
    queryFn: () => apiGet<MailList>("/api/mail-digest")
  });

  const variants = listQ.data?.variants ?? [];
  const selected = variants.find((v) => v.id === selectedId) ?? (!selectedId ? variants[0] ?? null : null);
  const selectedVariantId = selected?.id ?? null;

  useEffect(() => {
    const list = listQ.data?.variants;
    if (!list) return;
    if (selectedId && list.some((v) => v.id === selectedId)) return;
    if (selectedId && listQ.isFetching) return;
    const next = list[0]?.id ?? null;
    setSelectedId(next);
    if (next) writeLs(LS_VARIANT, next);
  }, [listQ.data, listQ.isFetching, selectedId]);

  const historyQ = useQuery({
    queryKey: ["mail-digest", "history", histStatus, histChannel, histVariantId],
    queryFn: () => {
      const q = new URLSearchParams();
      q.set("limit", "120");
      if (histStatus !== "all") q.set("status", histStatus);
      if (histChannel !== "all") q.set("target", histChannel);
      if (histVariantId !== "all") q.set("variantId", histVariantId);
      return apiGet<{ items: HistoryItem[] }>(`/api/mail-digest/history?${q.toString()}`);
    },
    enabled: pageTab === "history"
  });

  useEffect(() => {
    if (!selected || hydratedId === selected.id) return;
    setName(selected.name || selected.subjectTemplate || "Изменения плана ТО");
    setDescription(selected.description || "");
    setPeriodMode(selected.periodMode || "last7");
    setFromDate(selected.periodCustomFrom || customDefaults.from);
    setToDate(selected.periodCustomTo || customDefaults.to);
    setScheduleMode(selected.scheduleMode || "manual");
    setScheduleTime(selected.scheduleTime || "09:00");
    setWeekdays(selected.scheduleWeekdays?.length ? selected.scheduleWeekdays : [1, 2, 3, 4, 5]);
    setMonthDay(selected.scheduleMonthDay || 1);
    setIsActive(selected.isActive);
    setRecipients(selected.recipients);
    setColumns(selected.columns?.length ? selected.columns : DEFAULT_COLUMNS);
    setPreviewText("");
    setPreviewHtml("");
    setPreviewStats(null);
    setHydratedId(selected.id);
  }, [selected, hydratedId, customDefaults]);
  ...
  const composePayload = () => {
    ...
    const title = name.trim() || "Изменения плана ТО";
    return {
      name: title,
  ...
  const saveComposeM = useMutation({
    mutationFn: () => {
      if (!selectedVariantId) throw new Error("Сначала создайте вариант рассылки");
      return apiPut<MailVariant>(`/api/mail-digest/${selectedVariantId}`, composePayload());
    },
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["mail-digest", "list"] });
    }
  });

  const createM = useMutation({
    mutationFn: (body: Record<string, unknown>) => apiPost<MailVariant>("/api/mail-digest", body),
    onSuccess: async (row) => {
      setSelectedId(row.id);
      setHydratedId(null);
      writeLs(LS_VARIANT, row.id);
      setPageTab("variants");
      writeLs(LS_TAB, "variants");
      qc.setQueryData<MailList>(["mail-digest", "list"], (prev) => {
        if (!prev) return prev;
        const next: MailVariant = {
          ...row,
          lastSendAt: null,
          lastSendStatus: null,
          lastSendTarget: null
        };
        return { ...prev, variants: [next, ...prev.variants.filter((v) => v.id !== row.id)] };
      });
      await qc.invalidateQueries({ queryKey: ["mail-digest", "list"] });
    }
  });
  ...
  const deleteM = useMutation({
    mutationFn: (id: string) => apiDelete(`/api/mail-digest/${id}`),
    onSuccess: async (_d, id) => {
      if (selectedId === id) {
        setSelectedId(null);
        setHydratedId(null);
      }
      await qc.invalidateQueries({ queryKey: ["mail-digest"] });
    }
  });
  ...
  const previewM = useMutation({
    mutationFn: () => {
      if (!selectedVariantId) throw new Error("Сначала создайте вариант рассылки");
      return apiPost<MailPreview>(`/api/mail-digest/${selectedVariantId}/preview`, periodPayload());
    },
    onSuccess: (res) => {
  ...
  const sendM = useMutation({
    mutationFn: (target: "self" | "all") => {
      if (!selectedVariantId) throw new Error("Сначала создайте вариант рассылки");
      return apiPost<{ ok: true; messageId: string; recipients: string[]; subject: string }>(
        `/api/mail-digest/${selectedVariantId}/send`,
        {
          ...periodPayload(),
          recipients,
          subject: name.trim() || "Изменения плана ТО",
  ...
  const toggleActiveM = useMutation({
    mutationFn: (next: boolean) => {
      if (!selectedVariantId) throw new Error("Сначала создайте вариант рассылки");
      return apiPut<MailVariant>(`/api/mail-digest/${selectedVariantId}`, { isActive: next });
    },
    onSuccess: async (s) => {
  ...
  const smtpReady = listQ.data?.smtpReady ?? false;
  const effectivePeriod = scheduleMode !== "manual" && periodMode === "custom" ? "last7" : periodMode;
  const columnCatalog = listQ.data?.columnCatalog?.length ? listQ.data.columnCatalog : FALLBACK_CATALOG;
  ...
  const scheduledCount = variants.filter((v) => v.isActive && v.scheduleMode !== "manual").length;
  ...
  const selectVariant = (id: string) => {
    setSelectedId(id);
    setHydratedId(null);
    writeLs(LS_VARIANT, id);
    setWizardTab("what");
  };

  const createBlank = () => {
    createM.mutate({ name: variants.length ? `Рассылка ${variants.length + 1}` : "Изменения плана ТО" });
  };
  ...
  const duplicateSelected = () => {
    ...
    createM.mutate({
      ...composePayload(),
      name: copyName(name || selected.name),
      isActive: false
    });
  };
    ...
    <div className="mailDigestPage">
      <section className="massHero">
        ...
        <div className="massHeroStats" aria-label="Сводка рассылки">
          <span>
            <b>{variants.length}</b> вариантов
          </span>
          <span>
            <b>{activeCount}</b> активных
          </span>
          <span>
            <b>{listQ.data?.sentCount ?? 0}</b> отправок
          </span>
          {scheduledCount ? (
            <span>
              <b>{scheduledCount}</b> по расписанию
            </span>
          ) : null}
          <button type="button" className="btn btnPrimary" disabled={createM.isPending} onClick={createBlank}>
            + Новый вариант
          </button>
        </div>
      </section>
      ...
      {createM.error ? <div className="error">{String(createM.error.message || createM.error)}</div> : null}
      ...
      {pageTab === "history" ? (
          ...
          <div className="mailWizardBody">
            <div>
              ...
              <div className="muted adminHint">
                В журнале и ручные письма, и автоотправка по расписанию. Пустое письмо (нет изменений) тоже фиксируется.
              </div>
            </div>
            <div className="mailHistoryFilters">
              <div className="mailSeg" role="tablist" aria-label="Статус">
                {(
                  [
                    ["all", "Все"],
                    ["SENT", "Отправлено"],
                  ...
                  <button
                    key={id}
                    type="button"
                    className={`mailSegBtn ${histStatus === id ? "mailSegBtnOn" : ""}`}
                    onClick={() => setHistStatus(id)}
                  >
                    {label}
                  </button>
                ))}
              </div>
              <div className="mailSeg" role="tablist" aria-label="Способ">
                {(
                  [
                    ["all", "Все способы"],
                    ["manual", "Вручную"],
                  ...
                  <button
                    key={id}
                    type="button"
                    className={`mailSegBtn ${histChannel === id ? "mailSegBtnOn" : ""}`}
                    onClick={() => setHistChannel(id)}
                  >
                    {label}
                  </button>
                ))}
              ...
              {variants.length ? (
                <select
                  className="evInput"
                  value={histVariantId}
                  onChange={(e) => setHistVariantId(e.target.value)}
                  aria-label="Вариант рассылки"
                >
            ...
            <div className="muted adminHint">
              «Вручную» — отправка себе или всем из раздела. «Расписание» — автоматическая отправка.
            </div>
            ...
            {historyQ.data?.items.length ? (
                    ...
                    {historyQ.data.items.map((row) => (
                        <tr key={row.id}>
                          ...
                          <td>
                            ...
                            {row.stats ? (
                              <div className="muted">
                                аудит {row.stats.audits}; +{row.stats.added} / переносов {row.stats.prolonged} / отмен{" "}
                                {row.stats.cancelled}
                              </div>
                            ) : null}
                            {row.periodFrom && row.periodTo ? (
                              <div className="muted">
                                период {formatMskDay(row.periodFrom)} — {formatMskDay(row.periodTo)}
                              </div>
                            ) : null}
                          </td>
                          <td>{targetLabel(row.target)}</td>
                          <td>{row.target === "schedule" ? "Система" : row.actorEmail || "—"}</td>
                          <td>
              ...
              <div className="muted">Пока не было отправок по выбранным фильтрам.</div>
            ) : null}
          </div>
        </section>
      ) : (
        <>
          ...
          {!listQ.isLoading && !variants.length ? (
            <div className="sandboxesEmpty">
              Вариантов пока нет. Создайте первый — задайте период, колонки и получателей, затем сохраните или отправьте.
            </div>
          ) : null}

          {variants.length ? (
            <div className="mailVariantGrid">
              {variants.map((v) => (
                <button
                  key={v.id}
                  type="button"
                  className={`mailVariantCard ${v.id === selectedVariantId ? "mailVariantCardOn" : ""}`}
                  onClick={() => selectVariant(v.id)}
                >
                  <div className="mailVariantCardHead">
                    <div className="mailVariantCardTitle">{v.name}</div>
                    <span className={`mailStatusChip ${v.isActive ? "mailStatusChipOn" : "mailStatusChipOff"}`}>
                      {v.isActive ? "Активна" : "Пауза"}
                    </span>
                  </div>
                  {v.description ? <div className="mailVariantCardDesc">{v.description}</div> : null}
                  <div className="mailVariantCardMeta">
                    <span>{scheduleLabel(v)}</span>
                    <span>{v.recipients.length ? `${v.recipients.length} получ.` : "без получателей"}</span>
                    <span>
                      {v.lastSendAt
                        ? `${statusLabel(v.lastSendStatus || "")} ${formatMsk(v.lastSendAt)}`
                        : "ещё не отправлялась"}
                    </span>
                  </div>
                </button>
            ...
            <>
              <nav className="mailWizardSteps" aria-label="Шаги рассылки">
                {steps.map((s, i) => (
                  <button
                    key={s.id}
                    type="button"
                    className={`mailWizardStep ${wizardTab === s.id ? "mailWizardStepActive" : ""}`}
                    onClick={() => setWizardTab(s.id)}
                  >
                    <span className="mailWizardNum">{s.n}</span>
                    <span>{s.label}</span>
                ...
                {wizardTab === "what" ? (
                  <div className="mailWizardBody">
                    <div>
                      ...
                      <div className="muted adminHint">
                        В рассылку попадают изменения рабочего контура. Тема письма совпадает с названием варианта.
                      </div>
                    </div>

                    <label className="adminField">
                      <span className="muted">Название / тема письма</span>
                      <input
                        className="evInput"
                        value={name}
                    ...
                    <label className="adminField adminFieldGrow">
                      <span className="muted">Описание (необязательно)</span>
                      <textarea
                        className="mailDigestDesc"
                        rows={2}
                        value={description}
                        onChange={(e) => setDescription(e.target.value)}
                        placeholder="Для чего этот вариант, кому уходит"
                      />
                    </label>
                ...
                {wizardTab === "when" ? (
                  <div className="mailWizardBody">
                    <div>
                      ...
                      <div className="muted adminHint">Время указывается по Москве (MSK). На паузе расписание этого варианта не срабатывает.</div>
                    </div>
                          ...
                          <div className="mailWeekdays">
                            {WEEKDAYS.map((d) => (
                              <button
                                ...
                                onClick={() =>
                                  setWeekdays((prev) =>
                                    prev.includes(d.id) ? prev.filter((x) => x !== d.id) : [...prev, d.id].sort((a, b) => a - b)
                                  )
                                }
                              >
                                {d.label}
                    ...
                    <SwitchToggle
                      ...
                      label={isActive ? "Вариант активен" : "Вариант на паузе"}
                      hint="На паузе письма по расписанию этого варианта не уходят"
                    />
                  </div>
                ) : null}
                ...
                {wizardTab === "who" ? (
                  <div className="mailWizardBody">
                    <div>
                      ...
                      <div className="muted adminHint">Список только для этого варианта. Можно отправить письмо только себе.</div>
                    </div>
              ...
              <footer className="mailWizardFooter">
                <div className="mailWizardFooterLeft">
                  ...
                  <button type="button" className="btn" disabled={createM.isPending} onClick={duplicateSelected}>
                    Дублировать
                  </button>
                  <button
                    type="button"
                    ...
                    onClick={() => {
                      if (!selected) return;
                      if (!confirm(`Удалить вариант «${selected.name}»? История отправок сохранится.`)) return;
